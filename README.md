## QChain - Blockchain + Quantum Security

A student Junior Project combining blockchain and post-quantum cryptography.

### Meet our Team

- **Mohammed Bin Ali Maqqavi** - Project Manager
- **Mohammed Abdul Haris** - Technical Lead 
- **Mohammed Nihal** - Quality Assurance Lead 
- **Mohammed Obied** - Blockchain Specialist

### What We're Building

We're making a blockchain that uses special math (lattice cryptography) to be safe from quantum computers. 

Our goal: Make it secure AND fast.

### Project Files

- `docs/` - Our notes and reports
- `src/` - Our code
- `tests/` - Testing stuff

### Weekly Goals

**Week 1-3:** Research and planning  
**Week 4-6:** Choose which crypto to use  
**Week 7-10:** Build basic version  
**Week 11-13:** Test and fix  
**Week 14-16:** Final report and presentation  

### Contact

**Supervisors:**   
  - Dr. Manar Abu Talib -> Head Supervisor
  - Dr. Sohail Abbas -> Co Supervisor

---

Last updated: February 2026
